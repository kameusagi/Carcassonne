...
def tile_to_csv(xls, sheet_name ,folder_path):
    ...
    # CSVファイル名を決定
    csv_file_path = os.path.join(folder_path,f"cell_info.csv")
    
    # データフレームをCSVファイルとして保存
    result_df.to_csv(f"{csv_file_path}", index=False, encoding="utf-8-sig")
...
def process_excel_sheets_to_csv(excel_file_path):
    ...
    for sheet_name in xls.sheet_names:
        ...
        if sheet_name == "全体":
            ...
            folder_path = os.path.join(folder_path, "セル", sheet_name)
